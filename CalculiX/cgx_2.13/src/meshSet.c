  ...
  for (i=0; i<set[setNr].anz_l; i++)
  {
    ...
    if ((line[j].div>0)&&(!fillSurfFlag))
    {
      if (line[j].etyp==11)
      {
        ...
        {
          elem_define( anz->enext++, 11, &nl[u], 0, line[j].eattr );
	  ...
	  ebuf[1]=nl[u+2];
	  ebuf[2]=nl[u+1];
          elem_define( anz->enext++, 12, ebuf, 0, line[j].eattr );
          line[j].elem[k]=anz->emax;
          k++;
        }
        free(nl);
        ...
        for (c=0; c<cmax-1; c++)
        {
          for (a=0; a<amax-1; a++)
          {
            for (b=0; b<bmax-1; b++)
            {
              ...
              printf(" en2: %d %d %d %d \n",en[4],en[5],en[6],en[7]); 
#endif
              elem_define( anz->enext++, 1, en, 0, body[b_indx].eattr );
...
#endif
              elem_define( anz->enext++, 4, en, 0, body[b_indx].eattr );
              body[b_indx].elem[k]=anz->emax;
          ...
          if(body[b_indx].ori!='+')
  	{
          ...
          for (w=0; w<wmax; w++)
          {
            for (v=0; v<vmax; v++)
            {
              for (u=0; u<umax; u++)
              {
                nod( anz, &node, 0, anz->nnext++, npre[n_uvw[u*vmax*wmax+v*wmax+w]].nx, npre[n_uvw[u*vmax*wmax+v*wmax+w]].ny, npre[n_uvw[u*vmax*wmax+v*wmax+w]].nz, 0 );      
    ...
    if(e_enqire[e_enqire[i].nr].type==1)
    {
      for (j=0; j<8; j++)
      {
        ...
        else if ( nbuf[e_enqire[e_enqire[i].nr].nod[j]][0]==0 )  /* node not yet allocated */
        {
          ...
          nod( anz, &node, 0, anz->nnext, xn, yn, zn, 0 );
