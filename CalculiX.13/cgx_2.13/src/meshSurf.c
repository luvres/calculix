      ...
      j=0; for(i=0; i<e; i++)
      {
        ...
        if((nod_mesh2d[tri[j+1]]!=nod_mesh2d[tri[j]])&&(nod_mesh2d[tri[j+2]]!=nod_mesh2d[tri[j]])&&(nod_mesh2d[tri[j+2]]!=nod_mesh2d[tri[j+1]]))
        {
  	    ...
  	    ebuf[2]=nod_mesh2d[tri[j+2]];
            ...
            elem_define( anz->enext++, 8, ebuf, 0, surf[nr].eattr );
          }
          else
          {
    ...
    else
    {
      j=0; for(i=0; i<e; i++)
      {
        ...
        if((nod_mesh2d[tri[j+1]]!=nod_mesh2d[tri[j]])&&(nod_mesh2d[tri[j+2]]!=nod_mesh2d[tri[j]])&&(nod_mesh2d[tri[j+2]]!=nod_mesh2d[tri[j+1]]))
        {
  	    ...
  	    ebuf[1]=nod_mesh2d[tri[j+1]];
  	    ebuf[2]=nod_mesh2d[tri[j+2]];
            elem_define( anz->enext++, 7, ebuf, 0, surf[nr].eattr );
          }
          else
          {
    	    ebuf[0]=nod_mesh2d[tri[j+2]];
...
int mesh_tr3g( int s )
{
  ...
  //NOTHREAD if(sem_wait(&sem[8])) error("sem_wait");
        elem_define( anz->enext++, 7, ebuf, 0, surf[s].eattr );
...
void *meshSurf( void *arg)
{
  ...
  if(surf[j].o[0]==0) { thread_param->returnVal=-2; return(&thread_param->returnVal); }
...
*/
  ...
  //NOTHREAD if(sem_wait(&sem[8])) error("sem_wait");
              elem_define( anz->enext++, 7, ebuf, 0, surf[j].eattr );
  ...
  //NOTHREAD if(sem_wait(&sem[8])) error("sem_wait");
              elem_define( anz->enext++, 8, ebuf, 0, surf[j].eattr );
              surf[j].elem[k]=anz->emax;
              k++;
        ...
        for (b=0; b<div_b  ; b++)
        {
          for (a=0; a<div_a; a++)
          {
            ...
            if (surf[j].ori=='+')
            {
              ...
              ebuf[3]=n_ba[(b  )*amax + (a+1)];
  //NOTHREAD if(sem_wait(&sem[8])) error("sem_wait");
              elem_define( anz->enext++, 9 , ebuf, 0, surf[j].eattr );
  //NOTHREAD if(sem_post(&sem[8])) error("sem_post");
              surf[j].elem[k]=anz->emax;
            }
            else
  ...
  //NOTHREAD if(sem_wait(&sem[8])) error("sem_wait");
              elem_define( anz->enext++, 10, ebuf, 0, surf[j].eattr );
  //NOTHREAD if(sem_post(&sem[8])) error("sem_post");
              surf[j].elem[k]=anz->emax;
            }
            else
